...
//    std::string imagepath = image->Attribute("source");
    std::string imagepath = R"(D:\C\3sem_cpp\Necromancer\resources\levels\maps\merged.png)";
// пытаемся загрузить тайлсет
    sf::Image img;
    if (!img.loadFromFile(imagepath)) {
        std::cout << "Failed to load tile sheet."
                  << std::endl;//если не удалось загрузить тайлсет-выводим ошибку в консоль
        return false;
    }
...
Object Level::GetObject(std::string name) const {
...
std::vector<Object> Level::GetObjects(std::string name) const {
...
sf::Vector2<unsigned int> Level::getLevelSize() const {
    return {(unsigned int) width * tileWidth, (unsigned int) height * tileHeight};
}
